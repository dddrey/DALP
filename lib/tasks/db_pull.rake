namespace :db do
  ...
  task :dump do
    ...
    # system("ssh deploy@easteast.world 'PGPASSWORD=echo $EE_DATABASE_PASSWORD pg_dump -U deploy #{production['database']} -h #{db_conf['host']} -F t' > #{dumpfile}")
    system("ssh deploy@165.227.133.127 'pg_dump -U deploy #{production['database']} -h #{db_conf['host']} -F t' > #{dumpfile}")
    puts('Done!')
  end
